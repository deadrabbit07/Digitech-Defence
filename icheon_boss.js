...
document.querySelector('.abilitySet .check_btn').addEventListener('click', () => {
    stage.textContent = '???단계';
    ...
    if (setPower + setHeart + setSpeed === 30) {
        if (setHeart < 1) {
        ...
        document.querySelector('.abilitySet').setAttribute(`style`, 'opacity: 0;')
        setTimeout(() => {
            document.querySelector('.abilitySet').setAttribute(`style`, 'display: none;')
        }, 200);
...
function enemyOffensive() {
    ...
    enemy.classList.add('enemy_motion');

    console.log(friend.getBoundingClientRect().left)
    console.log(enemyHeart, frindHeart)

    frindHeart -= enemyPower;
    fTime = setTimeout(enemyOffensive, 100);
